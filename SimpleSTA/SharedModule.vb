...
Public Module SharedModule
    ...
    Public swtCurrentSwitch As Switch ' Global instance of the Switch object which represents the System Switch in the measurement hardware
    Public smCurrentSource As SourceMeter ' Global instance of the SourceMeter object which represents the Source Meter in the measurement hardware
    ...
    Public lngIPAddress As Long = ipv4StringToLong("192.168.1.114")
    ' -----------------------------------------------
    ...
    Public Function RunAuditCheck() As Boolean
        Try
            ...
            For Each acChannel In tfCurrentTestFile.AuditCheck.AuditChannels
                ...
                tsInfoFile.addSwitchEvent(acChannel.Card, 4)
            Next
    ...
    ' Description: 
    Public Sub GatherAuditReading(ByVal intRow As Integer, ByRef acChannel As AuditChannel)
        Try
            ...
            LogStringToFile(strLogPath, "Audit Check Row " & intRow)
            LogStringToFile(strLogPath, "node[1].channel.exclusiveclose('" & acChannel.Card & 2 & StrPad(acChannel.Column, 2) & "," & acChannel.Card & intRow & StrPad(acChannel.Column, 2) & "," & acChannel.Card & "912," & acChannel.Card & "91" & intRow & "')")
            ...
            switchDriver.System.DirectIO.FlushRead()
            acChannel.AddReading(AuditReading.ReadingFactory(dblVoltage, dblCurrent, cfgGlobal.ResistorNominalValues(intRow - 3), intRow))
        Catch ex As Exception
            Throw
        End Try
    End Sub
    ...
    Public Function UpdateSystemInfo() As Boolean
        Try
            ...
            ' @TODO: The line below can be removed once the swtCurrentSwitch object properly serializes in the test file
            tfCurrentTestFile.SwitchSerial = serialNo
            switchDriver.System.DirectIO.FlushRead()
            ...
            If Not tsInfoFile.GetSwitchBySerial(serialNo) Is Nothing Then
                swtCurrentSwitch = tsInfoFile.GetSwitchBySerial(serialNo)
                swtCurrentSwitch.Active = True
            Else
                ' otherwise create a new entry for the current switch
                swtCurrentSwitch = New Switch
                swtCurrentSwitch.SerialNumber = serialNo
                swtCurrentSwitch.Active = True
                swtCurrentSwitch.FirstTest = Now()
                ' Collect the switch model #
                DirectIOWrapper("print(localnode.model)")
                swtCurrentSwitch.ModelNumber = switchDriver.System.DirectIO.ReadString()
                switchDriver.System.DirectIO.FlushRead()
                ' Collect the model revision for the switch
                DirectIOWrapper("print(localnode.revision)")
                swtCurrentSwitch.Revision = switchDriver.System.DirectIO.ReadString()
                switchDriver.System.DirectIO.FlushRead()
                ' add the new switch to the tsInfoFile object / file
                tsInfoFile.AddSwitch(swtCurrentSwitch)
            End If
            ...
            switchDriver.System.DirectIO.FlushRead()
            ' If the source has already been registered in the TestSystem info file, set the value for the smCurrentSource in the 
            ' test file to the existing value and set it to active
            If Not tsInfoFile.GetSourceBySerial(serialNo) Is Nothing Then
                smCurrentSource = tsInfoFile.GetSourceBySerial(serialNo)
                smCurrentSource.Active = True
            Else
                ' otherwise create a new entry for the current source meter
                smCurrentSource = New SourceMeter
                smCurrentSource.SerialNumber = serialNo
                smCurrentSource.Active = True
                smCurrentSource.FirstTest = Now()
                ' Collect the source model
                DirectIOWrapper("print(node[2].model)")
                smCurrentSource.ModelNumber = switchDriver.System.DirectIO.ReadString()
                switchDriver.System.DirectIO.FlushRead()
                ' collect the source model revision
                DirectIOWrapper("print(node[2].revision)")
                smCurrentSource.Revision = switchDriver.System.DirectIO.ReadString()
                switchDriver.System.DirectIO.FlushRead()
                ' add the new sourcemeter to the test system info file / object
                tsInfoFile.AddSource(smCurrentSource)
            End If
            ...
            swtCurrentSwitch.Cards = aryCurrentCards
            tfCurrentTestFile.SystemSwitch = swtCurrentSwitch
            tfCurrentTestFile.SystemSource = smCurrentSource
    ...
    Public Sub AddCardInfo(ByVal intSlot As Integer)
        Try
            ...
            If Not strIDNString.Contains("Empty Slot") Then
                ...
                If Not swtCurrentSwitch.GetCardBySerial(strSerialNo) Is Nothing Then
                    ...
                    End If
                    aryCurrentCards(intCardIndex) = swtCurrentSwitch.GetCardBySerial(strSerialNo)
    ...
    Public Function ParseIDN(ByVal strIDNString As String, strValueDesired As String) As String
        ...
        End Try
    End Function
    Public Function ipv4StringToLong(ByVal strAddress As String) As Long
        Try
            Dim arySplitString As String() = Split(strAddress, ".")
            Dim lngOut As Long
            If (arySplitString.Length <> 4) Then
                Throw New Exception("Illegal ip address provided to ipv4StringToLong")
            Else
                lngOut = CLng(arySplitString(3)) + 256 * CLng(arySplitString(2)) + (256 ^ 2) * CLng(arySplitString(1)) + (256 ^ 3) * CLng(arySplitString(0))
            End If
            Return lngOut
        Catch ex As Exception
            Throw
        End Try
    End Function
    Public Sub LogStringToFile(ByVal strFilePath As String, strInput As String)
        Try
            Dim swWriter As StreamWriter
            swWriter = File.AppendText(strFilePath)
            swWriter.WriteLine(strInput & vbNewLine)
            swWriter.Close()
        Catch ex As Exception
            Throw
        End Try
